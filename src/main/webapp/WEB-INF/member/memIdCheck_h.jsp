  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
	...
	<script>
		...
		function idCheck() {
			...
			if(mid.trim() == "") {
				alert("아이디를 입력하세요!");
				childForm.mid.focus(); //자식폼
			}
			else {
				childForm.submit(); //자식폼 전송
			}
		}
		
		//부모창에 아이디값 보내기
		function sendCheck() {
			opener.window.document.myform.mid.value ='${mid}'; //부모한테 담음
			opener.window.document.myform.pwd.focus(); //다음꺼 비밀번호에 포커스
			window.close();
		}
	</script>
</head>
...
<div class="container">
	...
	<c:if test="${res != 1}">
		...
		<form name="childForm" method="post" action="${ctp}/memIdCheck.mem"> <!-- id체크해서 넘겨주는거 재사용 -->
			<p>
