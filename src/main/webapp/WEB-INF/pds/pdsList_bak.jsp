...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
	...
	<script>
		...
		//선택한 항목의 자료 삭제하기.. (프롬프트(prompt)창을 통하여 비밀번호를 입력받아서 처리)
		function pdsDelCheck(idx,fSName) {
			let ans = confirm("선택된 자료파일을 삭제하시겠습니까?");
			if(!ans) return false;
			
			let pwd = prompt("비밀번호를 입력하세요?"); //prompt:입력받는 명령어
...
<div class="container">
	...
	<table class="table table-borderless">
		<tr>
			...
			<td class="text-right">
				<a href="${ctp}/pdsInput.pds?part=${part}" class="btn btn-success">자료올리기</a> <!-- 자료의 주소보려고 button대신 a 씀 -->
			</td>
		</tr>
	</table>
    ...
    <tr class="table-dark text-dark">
		...
		<c:forEach var="vo" items="${vos}">
			<tr>
				<td>${curScrStartNo}</td>
        <td> <!-- 제목누르면 내용보여주기 -->
          <a href="${ctp}/pdsContent.pds?idx=${vo.idx}&pag=${pag}&part=${part}">${vo.title}</a> <!-- 현재글에대해서 봐야되니까 idx넘거야함 -->
        ...
        <td>
          ...
          <c:if test="${vo.hour_diff < 24}">		<!-- 24시간이 지나지 않았지만 현재시간~자정까찌는 '날짜:시간', 나머지는 '시간'만 출력 -->
    	      ${vo.day_diff > 0 ? fn:substring(vo.fDate,0,16) : fn:substring(vo.fDate,11,19)}
				...
				<td>${vo.downNum}</td>
				<td>
					<a href="#" onclick="modalView('${vo.title}','${vo.nickName}','${vo.mid}','${vo.part}','${vo.fName}','${vo.fSName}','${vo.fSize}','${vo.downNum}','${vo.fDate}')" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModal">모달창</a><br/> <!-- #은 현재창에서 처리하겠다, 여기다가 onclick값 넣어보면 혹시값안넘어갔을때 볼수있음 --> 
					전체다운<br/>
					<a href="javascript:pdsDelCheck('${vo.idx}','${vo.fSName}')" class="btn btn-danger btn-sm mb-1">삭제1</a><br/> <!-- 일반 지우는방법 , 통일시켜서 문자로 보냄(function)-->
					<a href="#" onclick="pdsDelCheckModal('${vo.idx}','${vo.fSName}','${part}','${pag}')" data-toggle="modal" data-target="#myPwdModal" class="btn btn-danger btn-sm">삭제2</a> <!-- 모달창 이용해서 받을거임 -->
				</td> <!-- 모달은 메세지차이떠도 뒤에 내용들이 다 보임 -->
			</tr>
