          ...
          <c:if test="${vo.hour_diff < 24}">		<!-- 24시간이 지나지 않았지만 현재시간~자정까찌는 '날짜:시간', 나머지는 '시간'만 출력 -->
    	      ${vo.day_diff > 0 ? fn:substring(vo.fDate,0,16) : fn:substring(vo.fDate,11,19)}
				...
				<td>
					...
					<h5><a href="${ctp}/pdsTotalDown.pds?idx=${vo.idx}" class="badge badge-success">전체다운</a><br/></h5>
					<h5><a href="javascript:pdsDelCheck('${vo.idx}','${vo.fSName}')" class="badge badge-warning">삭제1</a><br/></h5> <!-- 일반 지우는방법 , 통일시켜서 문자로 보냄(function)-->
					<h5><a href="#" onclick="pdsDelCheckModal('${vo.idx}','${vo.fSName}')" data-toggle="modal" data-target="#myPwdModal" class="badge badge-danger">삭제2</a></h5> <!-- 모달창 이용해서 받을거임 -->
