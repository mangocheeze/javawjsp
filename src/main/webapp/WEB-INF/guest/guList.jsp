...
<div class="container">
	...
	<br/>
		<table class="table table-borderless mb-0"> <!-- forEach바깥으로뺌 (안에넣으면 맨날 나오니까) --> <!-- 껍데기 -->
			<tr>
				<td>
					<c:if test="${sAMid != 'admin'}"><a href="${ctp}/adminLogin.gu" class="btn btn-sm btn-secondary">관리자</a></c:if> <!-- 현재 세션 id가 admin이 아닐땐 버튼 이름 관리자  -->
	...
	<c:forEach var="vo" items="${vos}" varStatus="st">  
		...
		<table class="table table-bordered"> <!-- 내용있는애??? -->
			...
			<tr>
				<th>방문소감</th>
				<td colspan="3">${fn:replace(vo.content, newLine, '<br/>')}</td> <!--  vo.content에 newLine을 <br/>로 치환(바꿔줘라)해라 -->
			</tr>
		</table>
		<br/>
	</c:forEach>
